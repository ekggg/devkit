...
EKG.registerWidget({
  ...
  handleEvent(event, state, ctx) {
    switch (event.type) {
      ...
      case "ekg.tip.sent":
        return event.data.currency === "BITS"
          ? addCheer(event, state, ctx)
          : addTip(event, state, ctx);
...
const push = (ctx, state, msg) => {
  const msgLimit =
    1 - (ctx.settings.messageLimit ? ctx.settings.messageLimitAmount : 100);
  return {
    ...state,
...
const deleteMessage = ({ data }, state, _ctx) => {
  state.messages = state.messages.filter((m) => m.id !== data.deletedMessageId);
  return state;
};

const deleteMessages = ({ data }, state, _ctx) => {
  state.messages = state.messages.filter(
    (m) => m.userId !== data.moderatedUserId,
  );
  return state;
};

const addMessage = ({ id, data }, state, ctx) => {
  // ignore set users
  ...
  return push(ctx, state, {
    ...
    expiresAt: ctx.settings.hideAfter
      ? ctx.now + ctx.settings.hideAfterAmount * 1000
      : null,
  });
};

const addFollower = (event, state, ctx) => {
...
const addNewSub = (event, state, ctx) => {
...
const addResub = (event, state, ctx) => {
  if (!ctx.settings.subscriberShow) return state;
...
const addGiftSub = (event, state, ctx) => {
  if (!ctx.settings.subscriberShow) return state;
...
const addCheer = (event, state, ctx) => {
  ...
  return push(ctx, state, {
    ...
    username: `${event.data.amountCents} bit${
      event.data.amountCents == 1 ? "" : "s"
    }`,
    status: ctx.random() > 0.5 ? "online" : "busy",
  });
};

const addTip = (event, state, ctx) => {
...
const cleanup = (event, state, ctx) => {
  const newMessages = state.messages.filter(
    (m) => !m.expiresAt || m.expiresAt >= ctx.now,
  );
...
const isEmotesOnly = (message, settings) => {
  let emoteCount = 0;
